...
- 数据库块位图（Data Bitmap）：记录数据块的分配状态（0 表示空闲，1 表示已分配）。
...
举个例子，假如一个 inode 有 12 个直接指针和 2 个一级间接指针，每个指针用 int32_t 类型存储，每个块的大小为 4K，
那么 12 个直接指针可以直接指向 12 * 4K = 48K 的数据，2 个一级间接指针可以指向 2 * (4096 / 4) = 2048 个数据块，
即 2 * 2048 * 4K = 8M 的数据。一共可以指向 8M + 48K 的数据。
...
比如直接读到 buffer，或者是当作目录，读取 inode + 名称等等。
...
理论上，你只要封装一下原来的 `disk_read/write` 即可，不会影响到其他代码，但是注意在文件系统关闭的时候确保所有的 cache 落盘。
